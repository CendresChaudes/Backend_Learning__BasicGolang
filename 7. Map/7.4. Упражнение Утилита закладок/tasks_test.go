...
func TestTask3(t *testing.T) {
	added := map[string]string{"docs": "https://go.dev"}
	if got, want := task3(added, "blog", "https://go.dev/blog"), 2; got != want {
		t.Errorf("получилось %d, нужно %d", got, want)
	}

	updated := map[string]string{"docs": "https://go.dev"}
	if got, want := task3(updated, "docs", "https://go.dev/doc"), 1; got != want {
		t.Errorf("получилось %d, нужно %d", got, want)
	}
}
